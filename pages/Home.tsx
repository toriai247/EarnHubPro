...
import { 
  Zap, Users, ArrowDownLeft, ArrowUpRight, Trophy, Wallet, Gamepad2
} from 'lucide-react';
  ...
  return (
    <div className="space-y-4 px-4 pb-20 pt-2">
      ...
      <div className="grid grid-cols-4 gap-2">
          {[
              { path: '/invite', icon: Users, label: 'Invite', show: isFeatureEnabled('is_invite_enabled') },
              { path: '/tasks', icon: Zap, label: 'Tasks', show: isFeatureEnabled('is_tasks_enabled') },
              { path: '/games', icon: Gamepad2, label: 'Games', show: isFeatureEnabled('is_games_enabled') },
              { path: '/leaderboard', icon: Trophy, label: 'Rank', show: true },
          ].map((item, i) => (
              item.show && (
                  <Link key={i} to={item.path} className="bg-card border border-border-base rounded p-3 flex flex-col items-center gap-2 hover:bg-input transition-colors">
